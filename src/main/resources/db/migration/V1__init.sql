-- =========================================================
-- V1__init.sql  (MySQL 8.0.16+)
-- 사용자 제공 초안(database.sql)을 정본으로 하고, 실행 불가 원인 2건만 수정했다.
--   (1) courses: fk_courses_creator 줄 끝 콤마 누락 → 문법 오류
--   (2) courses.reward_id 가 참조하는 rewards 테이블이 뒤에 생성됨 → rewards 를 앞으로 이동
-- 그 외 컬럼/타입/제약은 초안 그대로 유지한다. 어드민 운영에 필요한 추가 제약은
-- V2__admin_additions.sql 에서 additive 하게 붙인다.
-- =========================================================
...
-- ▼ 수정 (2): courses 가 reward_id 로 참조하므로 rewards 를 먼저 생성한다.
CREATE TABLE rewards (
  ...
  -- 원자적 차감 로직(애플리케이션)을 보완하는 DB 레벨 안전장치
...
-- place_id 또는 (latitude, longitude) 중 정확히 하나만 채워지도록 배타 처리
CREATE TABLE posts (
  id           BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  region_id    BIGINT UNSIGNED NOT NULL,
  user_id      BIGINT UNSIGNED NOT NULL,
